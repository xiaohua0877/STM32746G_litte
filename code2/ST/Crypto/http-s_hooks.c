...
#define  UPTIME_VAL_URL             "/uptime"
#define  RTC_VAL_URL                "/rtc_val"

#define  FORM_LOGOUT_FIELD_NAME     "Log out"

#if (NET_SECURE_CFG_EN == DEF_ENABLED)
#define  APP_CFG_DEMO_NAME          "Secure Webserver Demo"
#else
#define  APP_CFG_DEMO_NAME          "Webserver Demo"
#endif
...
extern  CLK_TS_SEC  App_SystemResetTime;
static  CPU_CHAR    UptimeStr[DEF_INT_32S_NBR_DIG_MAX];
static  CPU_CHAR    DateStr[CLK_STR_FMT_YYYY_MM_DD_HH_MM_SS_UTC_LEN];
                                    ...
                                    const  void            *p_hook_cfg)
{
    ...
    if (p_conn->Method == HTTP_METHOD_GET && p_conn->PathLenMax > 0u) {
        ...
        str_cmp = Str_Cmp_N(p_conn->PathPtr, UPTIME_VAL_URL, p_conn->PathLenMax);
        if (str_cmp == 0) {
            uptime = Clk_GetTS() - App_SystemResetTime;
            sprintf(UptimeStr, "%ld", uptime);
        ...
        str_cmp = Str_Cmp_N(p_conn->PathPtr, RTC_VAL_URL, p_conn->PathLenMax);
        if (str_cmp == 0) {
          (void)Clk_GetDateTime(&date_time);                    /* Update uC/Clk every 2 minutes.                       */
           if ((date_time.Sec == 0u) && (date_time.Min % 2u == 0u)) {
               App_SNTPc_SetClk((SNTPc_CFG *)&SNTPc_Cfg);
           }
             
           (void)Clk_DateTimeToStr(&date_time,
        ...
        if (str_cmp == 0) {
            HTTPs_RespBodySetParamStaticData(p_instance,        /* Set Parameters to tx response body in chunk.         */
                                             p_conn,
                                                       ...
                                                       CPU_INT16U       val_len_max)
{
    ...
    if (Str_Cmp_N(p_token, "os_version", 10) == 0) {
    ...
    } else if (Str_Cmp_N(p_token, "http_version", 13) == 0) {
       ...
       (void)Str_FmtNbr_Int32U(ver,  2, DEF_NBR_BASE_DEC, '0', DEF_NO, DEF_YES, &buf[6]);
        buf[8] = '\0';
    } else if (Str_Cmp_N(p_token, "dev_name",         8) == 0) {
        CPU_NameGet(&buf[0], &cpu_err);
    } else if (Str_Cmp_N(p_token, "dev_description", 15) == 0) {
        Str_Copy(&buf[0], APP_CFG_DEMO_NAME);
    } else if (Str_Cmp_N(p_token, "ip_address",      10) == 0) {
        Str_Copy(&buf[0u], App_Dflt_IP_Addr);
    } else if (Str_Cmp_N(p_token, "netmask",          8) == 0) {
        Str_Copy(&buf[0u], App_SubMsk);
    } else if (Str_Cmp_N(p_token, "gateway",          7) == 0) {
        Str_Copy(&buf[0u], App_Dflt_GW);
    }
                                           ...
                                           CPU_INT32U             *p_data_len)
{
    switch (status_code) {
        ...
        default:
             Str_Copy_N(p_file_str, HTTPs_HTML_DFLT_ERR_STR_NAME, file_len_max);
