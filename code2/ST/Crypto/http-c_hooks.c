...
extern                   int   STATIC_LOGO_GIF_LEN;
...
*               DEF_NO,     otherwise.
*
* Caller(s)   : HTTPcReq_QueryStrHook() via 'p_req->OnQueryStrTx()'.
*
                                   ...
                                   CPU_BOOLEAN         last_chunk)
{
    CPU_CHAR    *p_data_str;
    CPU_INT32U   buf_len;
    CPU_INT32U   data_consumed;
    CPU_INT32U   copy_size;
    CPU_INT08U  *p_conn_buf;
    CPU_INT16U   data_rem;


    p_data_str    = &HTTPcApp_Buf[0];
    buf_len       =  sizeof(HTTPcApp_Buf);
    data_consumed =  0u;
    p_conn_buf    = (CPU_INT08U *)p_data;
    data_rem      = data_len;   
    
    while (data_rem > 0) {
        copy_size     = DEF_MIN((buf_len - 1u), data_rem);
        Mem_Copy(p_data_str, p_conn_buf, copy_size);
        HTTPcApp_Buf[copy_size] = '\0';

        data_consumed += copy_size;
        data_rem      -= copy_size;
        p_conn_buf     = (CPU_INT08U *)p_data + data_consumed;

        HTTPc_APP_TRACE_INFO(("%s", p_data_str));

        if (last_chunk == DEF_YES && data_rem == 0u) {
